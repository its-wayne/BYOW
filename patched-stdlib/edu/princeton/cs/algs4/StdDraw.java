...
public final class StdDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
    ...
    ***************************************************************************/
    ...
    public static void clear(Color color) {
        ...
        offscreen.clearRect(0, 0, width, height);
        // NOTE (local patch): the onscreen buffer is intentionally NOT cleared here.
        // Clearing onscreen made clear() immediately blank the visible window, so under
        // double buffering every frame flashed to the background color before show() copied
        // the freshly-drawn offscreen buffer back. Clearing only the offscreen buffer keeps
        // clear() double-buffer-safe; immediate mode still works because draw() (below)
        // flushes via show() when double buffering is disabled.
